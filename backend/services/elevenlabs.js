...
async function getVoices() {
  if (!process.env.ELEVENLABS_API_KEY || process.env.ELEVENLABS_API_KEY === 'YOUR_API_KEY') {
     return []; // Return empty if no key, fallback to TTS
  }
  try {
    const resp = await axios.get('https://api.elevenlabs.io/v1/voices', {
      headers: { 'xi-api-key': process.env.ELEVENLABS_API_KEY }
    });
    return resp.data.voices.map(v => ({ id: v.voice_id, name: v.name, preview_url: v.preview_url }));
  } catch (error) {
...
async function generateVoice(text, outputPath, voiceId = null) {
...
module.exports = { generateVoice, getVoices };
